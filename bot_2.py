...
def settings_step1_param_and_button(callback_query: types.CallbackQuery):
    global last_message, m_id
    last_message = callback_query
    m_id = settings_step_1_func(last_message, bot, m_id)
    bot.answer_callback_query(callback_query.id)
...
def settings_step2_param_and_button(callback_query: types.CallbackQuery):
    global fl, last_message, m_id
    user_cmd = callback_query.data
    fl = True
    f = parse_txt_file('config_bot_2.txt')
    if 'hash' in user_cmd:
        m_id = bot.edit_message_text(chat_id=callback_query.from_user.id, message_id=m_id, text=f['change_hash_text'][int(user_cmd[-1])]).message_id
    else:
        m_id = step_start(f, callback_query.message, bot, m_id)
    last_message = callback_query
    bot.answer_callback_query(callback_query.id)
...
def initialize(message):
    ...
    try:
        ...
        callback_data = ''
    f = parse_txt_file('config_bot_2.txt')  # Подгрузка конфиг файлxxа
    if message.text in f['support'] or message.text in f['settings'] or message.text in f['instruction']:  # Сервис
        step_service(f, message.text, f['support'], f['settings'], f['instruction'], bot, m_id, message.chat.id)  # Методы из нижней сточки меню
        ...
        step_btn_pay(f, message, f['pay'].index(message.text), bot, m_id)  # step_2
        ...
        if message.text != f['lang'][0]:
            ...
            step_btn_pay(f, message, f['lang'].index(message.text), bot, m_id)  # step_2
        ...
        if fl:  # Если меняем параметр токен у пользователя
            ...
            fl = False
            m_id = settings_change_token_final_stage(message, f, bot, m_id)
