...
def step_btn_pay(data, message, idx, bot):  # Step_2 (After language)
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row(types.KeyboardButton(data[f'pay'][idx]))
...
def error_messages(data, message, bot, last_message):
    try:
        ...
        bot.send_message(message.chat.id, data['write_transaction'][get_lang(message, data)])
...
def settings_change_token_final_stage(message, f, bot):
    ...
    answer = f['all_save'][idx]
    bot.send_message(message.from_user.id, answer)
    bot.send_message(message.from_user.id, f['pump_funal_info'][idx])
